...
namespace Scool_cash_manager
{
    public partial class FrmSolvables : Form
    {
        ...
        private void BtnImprimer_Click(object sender, EventArgs e)
        {
            ...
            #region ajout des liste pour accompte, ....
            ...
            //les élèves ayant payé les avances >= à la moitiée
            pdfcell_colspan = new PdfPCell(new Phrase("Ayant payé + ou - l'accompte pour ce mois", police_entete_tableau))
            {
                Colspan = 5,
                HorizontalAlignment = V
            };
            table = GetTableAvanceSuperierMoitie();
            if (table.Rows.Count > 0)
                tableau.AddCell(pdfcell_colspan);
            j = 0;
            foreach (DataRow row in table.Rows)
